...
func _find_arcana_mod_directory(extracted_dir: String) -> String:
	...
	for subdir in mod_contents:
				...
				for item in json_result:
					if ("type" in item) and (item["type"] == "MOD_INFO"):
						...
						if "id" in info and info["id"] == "Arcana":
							...
							# CCB is DDA-compatible, so prefer DDA-specific package directories for both.
							if game in ["dda", "ccb"] and (subdir.to_lower().find("dda") != -1 or subdir.to_lower().find("dark") != -1):
								Status.post("Using DDA-compatible Arcana directory: %s" % subdir)
								break
		...
		if target_mod_dir != "" and ((game in ["dda", "ccb"] and (subdir.to_lower().find("dda") != -1 or subdir.to_lower().find("dark") != -1)) or
