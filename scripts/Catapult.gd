...
const VERSION_CHECK_URL = "https://api.github.com/repos/CrimsonCrossBunker/Catapult/releases/latest"
var _latest_version = ""
...
func _on_version_check_completed(result, response_code, headers, body):
	...
	if "tag_name" in response or "name" in response:
		_latest_version = response.get("tag_name", response.get("name", ""))
		var current_version = Settings.get_hardcoded_version()
		...
		if "html_url" in response:
			...
			_release_page_url = "https://github.com/CrimsonCrossBunker/Catapult/releases/latest"
...
func _numeric_version_parts(version: String) -> Array:
...
func _is_newer_version(latest: String, current: String) -> bool:
	var latest_parts = _numeric_version_parts(latest)
	var current_parts = _numeric_version_parts(current)
	var component_count = max(latest_parts.size(), current_parts.size())

	for index in range(component_count):
		var latest_component = latest_parts[index] if index < latest_parts.size() else 0
		var current_component = current_parts[index] if index < current_parts.size() else 0
