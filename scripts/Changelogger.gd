...
func download_pull_requests():
	...
	if _dda_pr_data.length() < 60 and game_selected == "dda":
		_ddaPullRequests.request(dda_pr_url,["Authorization: token REVOKED_GITHUB_TOKEN", "user-agent: GodotApp"])
	if _bn_pr_data.length() < 60 and game_selected == "bn":
		_bnPullRequests.request(bn_pr_url,["Authorization: token REVOKED_GITHUB_TOKEN", "user-agent: GodotApp"])
